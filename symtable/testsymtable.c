...
static void testBasics(void) {
    ...
    pcValue =
        (char *)SymTable_replace(oSymTable, acMantle, acFirstBase);
    ASSURE(pcValue == acCenterField);
    ...
    pcValue =
        (char *)SymTable_replace(oSymTable, "Clemens", acRightField);
    ASSURE(pcValue == NULL);

    pcValue =
        (char *)SymTable_replace(oSymTable, "Maris", acRightField);
    ASSURE(pcValue == NULL);
...
static void testLongKey(void) {
    ...
    for (i = 0; i < KEY_SIZE - 1; i++) acKeyA[i] = 'a';
    acKeyA[KEY_SIZE - 1] = '\0';

    for (i = 0; i < KEY_SIZE - 1; i++) acKeyB[i] = 'b';
    acKeyB[KEY_SIZE - 1] = '\0';
