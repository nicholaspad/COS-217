...
static int DynArray_isValid(DynArray_T oDynArray) {
    if (oDynArray->uPhysLength < MIN_PHYS_LENGTH)
        return 0;
...
static int DynArray_grow(DynArray_T oDynArray) {
    ...
    ppvNewArray = (const void **)realloc(oDynArray->ppvArray,
                                         sizeof(void *) * uNewLength);
    if (ppvNewArray == NULL)
        return 0;
...
static void DynArray_qsort(const void **ppvLo, const void **ppvHi,
    ...
    while (ppvRight <= ppvLeft) {
        while ((*pfCompare)(*ppvRight, pvPivot) < 0) ppvRight++;
        while ((*pfCompare)(pvPivot, *ppvLeft) < 0) ppvLeft--;
        if (ppvRight <= ppvLeft) {
...
void DynArray_sort(DynArray_T oDynArray,
    ...
    DynArray_qsort(&oDynArray->ppvArray[0],
...
int DynArray_search(DynArray_T oDynArray, void *pvSoughtElement,
    ...
    for (u = 0; u < oDynArray->uLength; u++)
        if ((*pfCompare)(oDynArray->ppvArray[u], pvSoughtElement) ==
            0) {
            *puIndex = u;
            return 1;
        }
...
static const void **DynArray_bsearchHelp(
    void *pvSoughtElement, const void **ppvLo, const void **ppvHi,
...
int DynArray_bsearch(DynArray_T oDynArray, void *pvSoughtElement,
    ...
    ppvElement = DynArray_bsearchHelp(
        pvSoughtElement, &oDynArray->ppvArray[0],
        &oDynArray->ppvArray[oDynArray->uLength - 1], pfCompare,
        &ppvInsert);
